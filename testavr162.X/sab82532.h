...
/* irqflags bits */
#define SAB_ALLS			0x00000001
#define SAB_XPR			    0x00000002
#define SAB_REGS_PENDING    0x00000004

#define SAB_XMEM_OFFSET 0x500

#define SAB_BASE_BAUD ( 2457600 / 16 )
...
#define SAB_CCR1_CM_1       (SAB_CCR1_CM0)
...
#define SAB_CEC_WAIT_TIME 50000
#define SAB_TEC_WAIT_TIME 200000

#define setBit(x,b) (*x |= b)
#define clrBit(x,b) (*x &=~b);
#define tstBit(x,b) (*x & b)
...
struct sab82532_async_rd_regs {
	uint8_t	rfifo[0x20];	/* Receive FIFO				*/
	uint8_t	star;		/* Status Register			*/
	uint8_t	__pad1;
	uint8_t	mode;		/* Mode Register			*/
	uint8_t	timr;		/* Timer Register			*/
	uint8_t	xon;		/* XON Character			*/
	uint8_t	xoff;		/* XOFF Character			*/
	uint8_t	tcr;		/* Termination Character Register	*/
	uint8_t	dafo;		/* Data Format				*/
	uint8_t	rfc;		/* RFIFO Control Register		*/
	uint8_t	__pad2;
	uint8_t	rbcl;		/* Receive Byte Count Low		*/
	uint8_t	rbch;		/* Receive Byte Count High		*/
	uint8_t	ccr0;		/* Channel Configuration Register 0	*/
	uint8_t	ccr1;		/* Channel Configuration Register 1	*/
	uint8_t	ccr2;		/* Channel Configuration Register 2	*/
	uint8_t	ccr3;		/* Channel Configuration Register 3	*/
	uint8_t	__pad3[4];
	uint8_t	vstr;		/* Version Status Register		*/
	uint8_t	__pad4[3];
	uint8_t	gis;		/* Global Interrupt Status		*/
	uint8_t	ipc;		/* Interrupt Port Configuration		*/
	uint8_t	isr0;		/* Interrupt Status 0			*/
	uint8_t	isr1;		/* Interrupt Status 1			*/
	uint8_t	pvr;		/* Port Value Register			*/
	uint8_t	pis;		/* Port Interrupt Status		*/
	uint8_t	pcr;		/* Port Configuration Register		*/
	uint8_t	ccr4;		/* Channel Configuration Register 4	*/
};

struct sab82532_async_wr_regs {
	uint8_t	xfifo[0x20];	/* Transmit FIFO			*/
	uint8_t	cmdr;		/* Command Register			*/
	uint8_t	__pad1;
	uint8_t	mode;
	uint8_t	timr;
	uint8_t	xon;
	...
	uint8_t	rfc;
	uint8_t	__pad2;
	uint8_t	xbcl;		/* Transmit Byte Count Low		*/
	uint8_t	xbch;		/* Transmit Byte Count High		*/
	uint8_t	ccr0;
	uint8_t	ccr1;
	uint8_t	ccr2;
	uint8_t	ccr3;
	uint8_t	tsax;		/* Time-Slot Assignment Reg. Transmit	*/
	uint8_t	tsar;		/* Time-Slot Assignment Reg. Receive	*/
	uint8_t	xccr;		/* Transmit Channel Capacity Register	*/
	uint8_t	rccr;		/* Receive Channel Capacity Register	*/
	uint8_t	bgr;		/* Baud Rate Generator Register		*/
	uint8_t	tic;		/* Transmit Immediate Character		*/
	uint8_t	mxn;		/* Mask XON Character			*/
	uint8_t	mxf;		/* Mask XOFF Character			*/
	uint8_t	iva;		/* Interrupt Vector Address		*/
	uint8_t	ipc;
	uint8_t	imr0;		/* Interrupt Mask Register 0		*/
	uint8_t	imr1;		/* Interrupt Mask Register 1		*/
	uint8_t	pvr;
	uint8_t	pim;		/* Port Interrupt Mask			*/
	uint8_t	pcr;
	uint8_t	ccr4;
};
...
typedef union sab82532_async_regs {
...
#define SABRegMem (*(sab82532_reg_mem_t *) (SAB_XMEM_OFFSET + SAB_CHAN_OFFSET(SAB_CHAN_A)))
...
#define SAB_82532_GetRegRamPtr(a) (SAB_XMEM_OFFSET + a)

#define SABReg(a)                 (*((SAB_REG_t *) SAB_82532_GetRegRamPtr(a)))
...
void SAB82532_CalcEbrg(uint32_t baud, int *, int *);
